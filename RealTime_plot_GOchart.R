...
##############################################################################/
#Example of plot for Gene Ontology annotation distribution####
##############################################################################/

op<-par(mfrow=c(1,2),mar=c(1,4.1,2,0.1))

level2<-read.table(file="GO_chart_data_20121107_1851_level2.txt",header=T,
                   sep="\t")
...
level3<-read.table(file="GO_modified_level3.txt", header=T, sep="\t")
